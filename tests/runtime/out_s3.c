...
void flb_test_s3_putobject_retry_limit_semantics(void)
{
    ...
    /*
     * Push 1 chunk then wait for 2 timer ticks (~1s each in mock mode).
     * Tick 1: upload_timeout expired, PutObject attempt 1 fails (failures=1)
     * Tick 2: failures(1) not > retry_limit(1), PutObject attempt 2 fails (failures=2)
     * Next encounter: failures(2) > retry_limit(1), chunk discarded
     */
    flb_lib_push(ctx, in_ffd, (char *) JSON_TD, (int) sizeof(JSON_TD) - 1);
    sleep(4);
